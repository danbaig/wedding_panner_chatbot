...
# These lists simulate the SQL files you provided.
budget_categories = [
    {"id": 2, "name": "Apparel"},
    {"id": 3, "name": "Reception"},
...
def parse_budget(budget_str: str) -> float:
    # Converts a budget string like "500k" to a float value.
...
#     Extracts only the title and profile_url fields and returns a markdown table.
#     If the user provided a wedding location, only vendors matching that location are included.
#     """
#     # Retrieve user's wedding location (if any).
#     user_location = ""
#     if "wedding_details" in st.session_state:
#         user_location = st.session_state.wedding_details.get("location", "").lower().strip()
...
#                 # If a location is provided, filter vendors by checking their address or state code.
#                 if user_location:
#                     vendor_address = data.get("address", "").lower()
#                     vendor_state = data.get("state_code", "").lower()
#                     if user_location not in vendor_address and user_location not in vendor_state:
#                         continue
...
#             table = header + "\n".join(rows)
#             return f"Sara: I've gathered some options for you:\n\n{table}\n\nWhat do you think? Would you like more details or to explore another service?"
#         else:
#             return "Sara: I'm sorry, I couldn't find any vendor profiles in your area right now."
#     else:
#         return "Sara: I'm sorry, I couldn't find any matching vendors at the moment."
@tool("vendor_retrieval")
def vendor_retrieval(query: str) -> str:
    """
    Retrieves and formats vendor information based on the query.
...
def budgeting_agent(query: str) -> str:
    ...
    if specific_category:
        ...
        # Overall allocation for this category is computed based on its proportion of the total ratio.
        ...
        # For each category, sum the ratios of its items.
        category_ratios = {}
        for cat in budget_categories:
...
def oracle_wedding(query: str) -> str:
    # Explicitly route queries containing "budget" to the budgeting agent.
